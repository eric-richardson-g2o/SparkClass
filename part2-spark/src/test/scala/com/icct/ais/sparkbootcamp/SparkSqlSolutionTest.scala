...
class SparkSqlSolutionTest {
  ...
  @Test
  def testOK() = {
    val spark: SparkSession = SparkSqlSolution.getSparkSession
    val data= SparkSqlSolution.loadData(spark)
    val claimIdCount = SparkSqlSolution.getDistinctClaimIds(data)
    assertEquals(118790, claimIdCount)

    val topDiagnosis:List[Row]=SparkSqlSolution.getTop10Diagnosis(data)
